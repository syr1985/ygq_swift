   ...
   open func percentUpdateDuringScrolling(_ percent:CGFloat){
        if percent > 1.0{
            ...
            UIView.animate(withDuration: 0.4, animations: {
                self.imageView.transform = CGAffineTransform(rotationAngle: CGFloat(-Double.pi+0.000001))
            })
            textLabel.text = textDic[.releaseToAction]
        }
        if percent <= 1.0{
            guard self.imageView.transform == CGAffineTransform(rotationAngle: CGFloat(-Double.pi+0.000001)) else{
                return
            }
            textLabel.text = textDic[.scrollToAction]
            UIView.animate(withDuration: 0.4, animations: {
